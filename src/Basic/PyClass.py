if __name__ == '__main__':
    ...
    python 的继承与 其他面向对象语言的语言类似。
        python 支持多继承；
        python 子类 override 它父类的任意方法
    namespace：名称空间，也就是变量名，或方法名的作用域。
    ...
    '''
    class 中的变量的使用需要注意，在 类中直接声明的变量相当于 java 中的 静态变量，是所有类的实例共享的。
    ...
    迭代器(iterators): 设计可迭代的类之前，我们先了解一下 for 循环的原理。
        实际上，for xx in xx 语句，会调用对象的 iter() 方法，这个函数会返回一个迭代器对象，定义有一个 __next__() 函数，
